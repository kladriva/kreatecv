...
<?= $this->section('title') ?>Étape 7: Hobbies<?= $this->endSection() ?>
...
<div class="form-container">
    <div class="progress">Étape 7/7</div>
    <h2>Centres d'intérêt (Hobbies)</h2>
        ...
        <div id="hobbies_wrapper">
            <div class="dynamic-block">
                <div class="form-group">
                    <label>Hobby</label>
                    <input type="text" name="hobbies[0][nom_hobby]" placeholder="Ex: Lecture, randonnée..." value="<?= set_value('hobbies[0][nom_hobby]') ?>">
                </div>
            </div>
        </div>
        <button type="button" id="add_hobby_btn" class="add-btn">+ Ajouter un hobby</button>

        <div class="nav-buttons">
            <a href="/cv/etape6" class="prev-btn">&larr; Précédent</a>
            <button type="submit" class="submit-btn">Terminer et créer mon CV</button>
        </div>
    <?= form_close() ?>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const wrapper = document.getElementById('hobbies_wrapper');
        const addButton = document.getElementById('add_hobby_btn');
        let hobbyIndex = 1;

        addButton.addEventListener('click', function () {
            const newBlock = wrapper.children[0].cloneNode(true);
            
            newBlock.querySelector('input[name="hobbies[0][nom_hobby]"]').name = `hobbies[${hobbyIndex}][nom_hobby]`;
