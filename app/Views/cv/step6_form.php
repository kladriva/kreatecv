...
<?= $this->section('title') ?>Étape 6: Langues<?= $this->endSection() ?>
...
<div class="form-container">
    <div class="progress">Étape 6/7</div>
    <h2>Langues</h2>
    ...
    <?= form_open('/cv/etape6') ?>
        <div id="langues_wrapper">
            <div class="dynamic-block">
                <div class="form-group">
                    <label>Langue</label>
                    <input type="text" name="langues[0][nom_langue]" placeholder="Ex: Anglais" value="<?= set_value('langues[0][nom_langue]') ?>">
                </div>
                <div class="form-group">
                    <label>Niveau</label>
                    <select name="langues[0][niveau]">
                        <option value="Notions" <?= set_select('langues[0][niveau]', 'Notions') ?>>Notions</option>
                        <option value="Intermédiaire" <?= set_select('langues[0][niveau]', 'Intermédiaire') ?>>Intermédiaire</option>
                        <option value="Courant" <?= set_select('langues[0][niveau]', 'Courant', true) ?>>Courant</option>
                        <option value="Bilingue / Natif" <?= set_select('langues[0][niveau]', 'Bilingue / Natif') ?>>Bilingue / Natif</option>
                    </select>
                </div>
            </div>
        </div>
        <button type="button" id="add_langue_btn" class="add-btn">+ Ajouter une langue</button>

        <div class="nav-buttons">
            <a href="/cv/etape5" class="prev-btn">&larr; Précédent</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const wrapper = document.getElementById('langues_wrapper');
        const addButton = document.getElementById('add_langue_btn');
        let langueIndex = 1;

        addButton.addEventListener('click', function () {
            const newBlock = wrapper.children[0].cloneNode(true);
            
            newBlock.querySelector('input[name="langues[0][nom_langue]"]').name = `langues[${langueIndex}][nom_langue]`;
            newBlock.querySelector('select[name="langues[0][niveau]"]').name = `langues[${langueIndex}][niveau]`;
